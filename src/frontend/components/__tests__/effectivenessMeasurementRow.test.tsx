...
describe('EffectivenessMeasurementRow', () => {
  ...
  describe('Component Rendering', () => {
    ...
    it('renders tooltip with correct content', () => {
      ...
      expect(tooltipHost.prop('content')).toEqual(
        <div dangerouslySetInnerHTML={{ __html: 'This is a test tooltip' }} />
      );
    });
    ...
    it('renders voting buttons with correct aria-labels', () => {
      ...
      for (let i = 1; i <= 10; i++) {
        const buttons = wrapper.find(DefaultButton).filterWhere(
          button => button.prop('aria-label') === i.toString()
        );
  ...
  describe('User Interactions', () => {
    it('calls onSelectedChange when a voting button is clicked', () => {
      ...
      const button3 = wrapper.find(DefaultButton).filterWhere(
        button => button.prop('aria-label') === '3'
      ).first();
    ...
    it('updates internal state when voting button is clicked', () => {
      ...
      const button7 = wrapper.find(DefaultButton).filterWhere(
        button => button.prop('aria-label') === '7'
      ).first();
    ...
    it('handles multiple button clicks correctly', () => {
      ...
      const button2 = wrapper.find(DefaultButton).filterWhere(
        button => button.prop('aria-label') === '2'
      ).first();
      const onClick2 = button2.prop('onClick') as () => void;
      onClick2();

      const button8 = wrapper.find(DefaultButton).filterWhere(
        button => button.prop('aria-label') === '8'
      ).first();
  ...
  describe('Icon States', () => {
    it('shows CircleFill for selected button and CircleRing for others', () => {
      ...
      const button5 = wrapper.find(DefaultButton).filterWhere(
        button => button.prop('aria-label') === '5'
      ).first();
      expect(button5.prop('iconProps')).toEqual({ iconName: 'CircleFill' });

      for (let i = 1; i <= 10; i++) {
        if (i !== 5) {
          const button = wrapper.find(DefaultButton).filterWhere(
            button => button.prop('aria-label') === i.toString()
          ).first();
  ...
  describe('All Voting Button Coverage', () => {
    it('tests all 10 voting buttons individually', () => {
      ...
      for (let i = 1; i <= 10; i++) {
        ...
        const button = wrapper.find(DefaultButton).filterWhere(
          btn => btn.prop('aria-label') === i.toString()
        ).first();
  ...
  describe('Tooltip Button Coverage', () => {
    it('renders tooltip button with correct properties', () => {
      ...
      const tooltipButton = wrapper.find(DefaultButton).filterWhere(
        button => button.prop('iconProps')?.iconName === 'Error'
      );
    ...
    it('renders tooltip with complex HTML content', () => {
      ...
      expect(tooltipHost.prop('content')).toEqual(
        <div dangerouslySetInnerHTML={{ __html: complexTooltip }} />
      );
    });
  });
});
