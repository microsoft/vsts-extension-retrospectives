...
const processImportedData = async (importedData: IExportImportDataSchema[]) => {
  ...
  for (const dataToProcess of importedData) {
    ...
    const newBoard = await boardDataService.createBoardForTeam(team.id, oldBoard.title, oldBoard.maxVotesPerUser, oldBoard.columns, oldBoard.isIncludeTeamEffectivenessMeasurement, oldBoard.shouldShowFeedbackAfterCollect, oldBoard.isAnonymous, oldBoard.startDate, oldBoard.endDate, oldBoard.permissions, oldBoard.teamAssessmentQuestions);
...
export const ExtensionSettingsMenu: React.FC<ExtensionSettingsMenuProps> = ({ currentUserIsTeamAdmin = false, allWorkItemTypes = emptyWorkItemTypes, allowedActionItemWorkItemTypeNames = emptyWorkItemTypeNames, onSaveAllowedActionItemWorkItemTypes }) => {
  ...
  return (
    <div className="extension-settings-menu" ref={menuRootRef} onKeyDown={trackActivity} onMouseMove={trackActivity} onTouchStart={trackActivity}>
      ...
      <dialog className="whats-new-dialog dialog-width-md" aria-label="What is New" ref={whatsNewDialogRef} onCancel={() => whatsNewDialogRef.current!.close()}>
        ...
        <div className="subText">This release focused on supporting the 'Add work item' option with custom work items.</div>
        <div className="subText li">Refactored 'Add work item' selection to ensure all work item types are visible even with multiple custom work item types for Act tab and only requirement backlog work item types are shown in Focus mode.</div>
