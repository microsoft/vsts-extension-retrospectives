...
describe("localization", () => {
  ...
  it("preserves placeholders when interpolation values are missing", () => {
    ...
    expect(t("feedback_board_create_example", { other: "value" })).toBe("{{date}} Retrospective");
  });

  it("replaces placeholders when interpolation values are provided", () => {
    initializeLocale("en-US");

    expect(t("feedback_board_create_example", { date: "May 2026" })).toBe("May 2026 Retrospective");
  });
